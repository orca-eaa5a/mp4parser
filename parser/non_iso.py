    ...
    def __init__(self, fp, header):
        ...
        try:
            ...
            while bytes_left > 7:
                ...
                current_box = parser.iso.box_factory(fp, current_header)
        ...
        try:
            ...
            while fp.tell() < (self.start_of_box + self.size):
                ...
                if this_descriptor_dict['tag_id'] == 4:
                    ...
                    this_descriptor_dict['mp4ra_registered_type'] = parser.mpeglookups.mp4ra_table[mp4ra_type] \
                        if mp4ra_type in parser.mpeglookups.mp4ra_table else mp4ra_type
                    type_byte = int.from_bytes(payload[1:2], byteorder='big')
                    es_type = type_byte >> 2
                    this_descriptor_dict['es_type'] = parser.mpeglookups.es_table[es_type] \
                        if es_type in parser.mpeglookups.es_table else es_type
                    ...
                    if this_descriptor_dict['specific_info_flag']:
                        ...
                        if es_type == 5:
                            ...
                            if sound_codec == 31:
                                ...
                                bits_read += field_size
                            decoder_specific_dict['sound_codec'] = parser.mpeglookups.sound_codec_table[sound_codec]
                            field_size = 4
                            decoder_specific_dict['freq_id'] = parser.mpeglookups.freq_table[
                            ...
                            decoder_specific_dict['channels'] = parser.mpeglookups.sound_channel_table[sound_chan] \
                                if sound_chan in parser.mpeglookups.sound_channel_table else sound_chan
                        else:
    ...
    def __init__(self, fp, header):
        ...
        try:
            ...
            while bytes_left > 7:
                ...
                current_box = ItemBox(fp, current_header)
                self.children.append(current_box)
                bytes_left -= current_box.size
        finally:
            fp.seek(self.start_of_box + self.size)
